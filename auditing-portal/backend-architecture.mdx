...
| `ScannerModule` | Stellar and Solana scanner providers and scheduled scanner service |
...
| Reports | `/api/applications/:foreignId/case-reports`, `/api/applications/:foreignId/case-reports/:reportId/download` | `reports:create`, `reports:list`, `reports:download` |
...
| Team and admin | `/api/admin/team/...` | Organization owner and team-management checks |

## Scheduled jobs

| Job | Service | Purpose |
| --- | --- | --- |
| Stellar scanner tick | `LedgerScannerSchedulerService` | Scan configured Stellar chain names for registered contracts |
...
The Stellar/Soroban scanner path and the Solana confidential-token scanner path share workflow tables after normalization. The chain adapters remain separate.
...
| Mechanism | Purpose |
| --- | --- |
| `WorkosAuthGuard` | Verifies authenticated session or JWT |
