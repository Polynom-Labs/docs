...
flowchart LR
  ...
  db["Partner database"]
  sdk["Arcane SDK"]
  arcane["Arcane indexer and relayer"]
  rpc["Chain RPC"]
...
| Reconciliation | Link SDK outputs and transaction signatures back to product ids |
...
2. Create or retrieve the managed wallet row.
...
1. Resolve recipient managed wallet or private recipient key.
2. Scan spendable state.
3. Build private transfer.
4. Submit through the relayer.
5. Persist status history and transaction signature.
...
- Log access to support and disclosure workflows.
