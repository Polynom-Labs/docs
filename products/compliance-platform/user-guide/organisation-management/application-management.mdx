...
- the application creator receives application-administrator permissions when
  the creator has a synchronized organization member record;
...
The current application record does not maintain a separate `Setup needed`,
`Workspace active`, or `Suspended` status. The management service distinguishes
between applications that are available in the organization registry and
applications that have been revoked.

Organization verification and application integration readiness are separate
controls. A listed application is not necessarily ready for production use;
the associated contract, access assignments, integration configuration, and
operational controls must also be ready.
...
<Columns cols={2}>
  ...
  <Card
    ...
    icon="blocks"
    href="/products/compliance-platform/integration/stellar/integration-overview"
  >
    Continue to the Stellar integration documentation.
  </Card>
</Columns>
