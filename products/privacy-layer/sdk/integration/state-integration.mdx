...
| --- | --- | --- |
| **In-memory** | `@arcanetech/privacy-sdk-state-memory` | Tutorials, prototypes, unit tests |
| **Redux** | `@arcanetech/privacy-sdk-state-redux` | Production React apps already using Redux Toolkit |
...
**Pros:** zero extra dependencies, fast setup, ideal for [Quick start](/products/privacy-layer/sdk/integration/quick-start).
...
```bash
npm install @arcanetech/privacy-sdk-state-redux @reduxjs/toolkit react-redux
```

The Redux adapter is **optional**. Many teams start with in-memory state in tests, then adopt Redux for production persistence.

Full store wiring, provider, and localStorage persistence live in [Setup](/products/privacy-layer/sdk/application-development/setup) — copy that pattern rather than re-deriving it here.
