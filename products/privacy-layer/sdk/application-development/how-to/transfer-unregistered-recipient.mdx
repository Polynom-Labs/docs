...
description: Transfer to a recipient who is not yet registered, with onboarding at execute time
---
...
Unregistered recipients need `resolveTransferRecipientAtExecute` and `buildTransferOnboardingAtExecute` on `transactEnvironment`. Add them during client bootstrap (see [Setup](/products/privacy-layer/sdk/application-development/setup)).

```ts
import {
  buildTransferOnboardingAtExecute,
  generateTemporaryRecipientPrivateAddress,
} from '@arcanetech/privacy-sdk-stellar/transact';
...
<CardGroup cols={2}>
  <Card title="Operations — Pending claim" icon="arrow-right-arrow-left" href="/products/privacy-layer/sdk/concepts/operations#transfer-to-unregistered-recipient-pending-claim">
  ...
  <Card title="Domain model" icon="sitemap" href="/products/privacy-layer/sdk/concepts/domain-model#pending-claims">
